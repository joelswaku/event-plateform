terraform {
  ...
  backend "s3" {
    bucket         = "liteevent-terraform-state-prod"
...
module "elasticache" {
  ...
  num_cache_nodes    = 2 # 2 nodes for production
}
...
# ECR Lifecycle Policies (keep more images in production)
...
# ========================================
# CloudFront Module - DISABLED (Account verification required)
# ========================================
# ENABLE AFTER: AWS account is verified for CloudFront (contact AWS Support)
# module "cloudfront" {
...
module "github_oidc" {
  ...
  enable_secrets_access = true # Production CI may need to read secrets for migrations
}
