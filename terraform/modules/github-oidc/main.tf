...
# IAM Role for GitHub Actions - ECR Push & ECS Deploy
...
# Policy for running ECS tasks (migrations, one-off commands)
resource "aws_iam_role_policy" "github_actions_ecs_exec" {
  name = "${var.project_name}-${var.environment}-github-ecs-exec-policy"
  role = aws_iam_role.github_actions.id
...
# Policy for Secrets Manager (read only - for migrations that need secrets)
resource "aws_iam_role_policy" "github_actions_secrets" {
  count = var.enable_secrets_access ? 1 : 0
  name  = "${var.project_name}-${var.environment}-github-secrets-policy"
  role  = aws_iam_role.github_actions.id
