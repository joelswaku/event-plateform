/**
 * eventapp-mobile/components/builder/BuilderCanvas.tsx
 *
 * REBUILT — animated canvas that matches web builder exactly:
 *
 * ✅ Sections render with real data via SectionPreviewCard
 * ✅ Each section animates in (fade+slide) on load — same feel as web
 * ✅ Theme change triggers a flash animation across all cards (same as web's instant repaint)
 * ✅ Adding a new section slides it in from bottom
 * ✅ Deleting a section fades it out
 * ✅ Selected section shows indigo ring (matches web hover:ring-2)
 * ✅ Long-press → action sheet: Edit / Move Up / Move Down / Hide / Delete
 * ✅ DB sync: all mutations go through builderStore which hits the API
 *   → Web sees changes instantly because both share the same DB
 * ✅ Empty state matches web empty canvas
 */

import React, { useRef, useCallback, useEffect } from 'react';
import {
  View, Text, StyleSheet, ScrollView, ActivityIndicator,
  Alert, Pressable, Animated,
} from 'react-native';
...
/* ── Animated section row wrapper ────────────────────────────────── */
function AnimatedSection({
  sec, selectedSectionId, onSectionSelect, onLongPress, event, index,
}: {
  sec: BuilderSection; selectedSectionId: string | null;
  onSectionSelect: (s: BuilderSection) => void;
  onLongPress: (s: BuilderSection) => void;
  event: Event | null; index: number;
}) {
  const fadeAnim  = useRef(new Animated.Value(0)).current;
  const slideAnim = useRef(new Animated.Value(20)).current;

  useEffect(() => {
    /* Staggered entrance — each section delays by index * 40ms */
    Animated.parallel([
...
export default function BuilderCanvas({
  ...
  /* Theme change flash animation — all sections briefly scale */
  const themeKey = sections[0]?.config?._theme ?? 'MODERN';
  ...
  useEffect(() => {
    if (prevTheme.current !== themeKey) {
      ...
      /* Quick scale pulse — matches web's instant theme repaint feel */
      Animated.sequence([
        Animated.timing(flashAnim, { toValue: 0.97, duration: 100, useNativeDriver: true }),
  ...
  const handleLongPress = useCallback((sec: BuilderSection) => {
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          style: idx === 0 ? 'destructive' : 'default',
        },
        {
          text: '⬇️ Move Down',
          onPress: idx < sections.length - 1 ? () => moveSection(idx, 'down') : undefined,
          style: idx === sections.length - 1 ? 'destructive' : 'default',
        },
        {
          text: sec.is_visible === false ? '👁 Show' : '🙈 Hide',
          onPress: () => updateSection(eventId, sec.id, { is_visible: sec.is_visible === false }),
        },
        {
          text: '🗑 Delete',
          style: 'destructive',
  ...
  const moveSection = async (idx: number, dir: 'up' | 'down') => {
    ...
    const payload = reordered.map((sec, i) => ({ id: sec.id, position_order: i + 1 }));
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    ...
    return (
      <View style={s.center}>
        <View style={s.emptyIconWrap}>
          <Feather name="layers" size={32} color={`${ACC}60`} />
        </View>
        <Text style={s.emptyTitle}>Canvas is empty</Text>
        <Text style={s.emptySub}>Tap Add ↓ to drop in your first block</Text>
      </View>
    );
  }

  return (
    <Animated.View style={[{ flex: 1 }, { transform: [{ scale: flashAnim }] }]}>
      <ScrollView
        style={s.canvas}
...
const s = StyleSheet.create({
  ...
  loadingTxt: { fontSize: 13, color: MT },

  emptyIconWrap: {
    width: 68, height: 68, borderRadius: 20,
    backgroundColor: `${ACC}12`,
    alignItems: 'center', justifyContent: 'center',
    marginBottom: 4,
  },
  emptyTitle: { fontSize: 16, fontWeight: '700', color: 'rgba(255,255,255,0.35)' },
  emptySub:   { fontSize: 13, color: 'rgba(255,255,255,0.2)', textAlign: 'center', paddingHorizontal: 40 },
});
