...
// Next.js App Router wraps page.js in Suspense automatically, so this is fine.
export default function LoginPage() {
  const { login, isLoading } = useAuthStore();
  const router       = useRouter();
  const searchParams = useSearchParams();                    // ← FIX: was missing
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      // ── FIX: respect ?redirect= param, fall back to /dashboard ──────────
  ...
  return (
    ...
    >
      ...
      {/* Google */}
      <div className="mt-8">
        <GoogleLoginButton redirectTo={searchParams.get("redirect") || "/dashboard"} />
      </div>
