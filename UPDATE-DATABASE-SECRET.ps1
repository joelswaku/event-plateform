Write-Host "Updating Database Secret with SSL Configuration..." -ForegroundColor Cyan
Write-Host ""
...
$newSecretValue = @'
{
  ...
  "url": "postgresql://liteevent_admin:LiteEvent2026Pass@liteevent-production-postgres.ck9ycc66044g.us-east-1.rds.amazonaws.com:5432/liteevent_production?sslmode=require"
}
'@
...
Write-Host ""
Write-Host "After saving, go back to ECS and update the service!" -ForegroundColor Cyan

# Copy to clipboard
Set-Clipboard -Value $newSecretValue
Write-Host "JSON copied to clipboard - just paste it!" -ForegroundColor Green
